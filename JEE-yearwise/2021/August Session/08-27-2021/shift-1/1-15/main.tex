...
\end{enumerate}
\item If for $x,y \in \mathbf{R},x>0$, $y=\log _{10}x+\log _{10}x^{1/3}+\log _{10}x^{1/9}+\ldots$ upto $\infty$ terms and $\frac{2+4+6+\ldots+2y}{3+6+9+\ldots+3y} = \frac{4}{\log _{10}x}$, then the ordered pair \brak{x,y} is equal to:\hfill  [Aug 2021]
\begin{enumerate}
    \item [a.] $\brak{10^6,6}$
    \item [b.] $\brak{10^4,6}$
    \item [c.] $\brak{10^2,3}$
    \item [d.] $\brak{10^6,9}$
\end{enumerate}
\item Let A be a fixed point \brak{0, 6} and B be a moving
...
\end{enumerate}
\item If the matrix $$\begin{bmatrix}
    0&2\\K&-1
...
\begin{enumerate}
    ...
    \item [d.] 1
\end{enumerate}
\item The distance of the point \brak{1, -2, 3} from the plane $x - y + z = 5$ measured parallel to a line, whose
...
\end{enumerate}
\item Let $y=y\brak{x}$ be the solution of the differential equation $$\frac{dy}{dx}=2\brak{y+2\sin x-5}x-2\cos{x}$$ such that y\brak{0}=7, then $y\brak{\pi}$is equal to\hfill  [Aug 2021]
\begin{enumerate}
    \item [a.] $2e^{\pi ^2}+5$
    \item [b.] $e^{\pi ^2}+5$
    \item [c.] $3e^{\pi ^2}+5$
...
\item Let us consider a curve, $y = f\brak{x}$ passing through the point \brak{-2,2} and the slope of the tangent to the curve at any point \brak{x,f\brak{x}} is given by $f\brak{x}+xf'\brak{x}=x^2$ then:\hfill  [Aug 2021]
\begin{enumerate}
